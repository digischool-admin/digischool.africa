# 🌐 OFFICIAL LINKS - DigiSchool Africa

**Date**: January 17, 2026  
**Version**: 2.0.0  
**Status**: ✅ PRODUCTION READY

---

## 🏠 MAIN SITE URLS

### Public Pages
- **Homepage**: https://digischool.africa/
- **B2C Catalog**: https://digischool.africa/b2c.html
- **B2B Corporate**: https://digischool.africa/companies.html
- **About Us**: https://digischool.africa/about.html
- **Contact**: https://digischool.africa/contact.html

### B2C User Journey
- **Checkout**: https://digischool.africa/b2c-checkout.html
- **Access Restoration**: https://digischool.africa/b2c-access.html
- **User Dashboard**: https://digischool.africa/user-dashboard.html
- **Course Overview**: https://digischool.africa/b2c-learn.html
- **Module Learning**: https://digischool.africa/b2c-module.html
- **Assessment (Recommender)**: https://digischool.africa/b2c-assessment.html

### Admin & Support
- **Admin Dashboard**: https://digischool.africa/admin.html
  - Password: `DigiSchool2026!`
- **WhatsApp Assistant**: https://digischool.africa/whatsapp-assistant.html

---

## 📱 SOCIAL MEDIA (OFFICIAL)

### Primary Channels
- **Facebook**: https://facebook.com/digischoolafrica
- **LinkedIn**: https://linkedin.com/company/digischoolafrica
- **TikTok**: https://tiktok.com/@digischoolafrica
- **YouTube**: https://youtube.com/@digischoolafrica
- **Telegram**: https://t.me/digischoolafrica

### Direct Contact
- **WhatsApp**: https://wa.me/2250505111102
- **Email**: support@digischool.africa
- **Business Email**: contact@digischool.africa

---

## 💳 PAYMENT METHODS (Mobile Money - Côte d'Ivoire)

**IMPORTANT**: Payment numbers are ONLY shown in b2c-checkout.html after cart confirmation

- **Orange Money**: +225 07 14 67 82 89
- **MTN MoMo**: +225 05 65 23 14 03
- **Moov Money**: +225 01 51 66 68 01
- **Wave**: +225 01 51 66 46 53

---

## 📚 ALL 9 COURSES (Direct URLs)

### 1. Leadership & Management
- **URL**: https://digischool.africa/b2c-learn.html?course=leadership-management
- **Slug**: `leadership-management`
- **Duration**: 5 days (8 modules)
- **Price**: 245,000 FCFA (pack)

### 2. Gestion de projet (PMP)
- **URL**: https://digischool.africa/b2c-learn.html?course=gestion-projet-pmp
- **Slug**: `gestion-projet-pmp`
- **Duration**: 10 days (8 modules)
- **Price**: 285,000 FCFA (pack)

### 3. Stratégie & Exécution
- **URL**: https://digischool.africa/b2c-learn.html?course=strategie-execution
- **Slug**: `strategie-execution`
- **Duration**: 4 days (8 modules)
- **Price**: 245,000 FCFA (pack)

### 4. Finance pour non-financiers
- **URL**: https://digischool.africa/b2c-learn.html?course=finance-non-financiers
- **Slug**: `finance-non-financiers`
- **Duration**: 3 days (8 modules)
- **Price**: 180,000 FCFA (pack)

### 5. Vente B2B & Négociation
- **URL**: https://digischool.africa/b2c-learn.html?course=vente-b2b-negociation
- **Slug**: `vente-b2b-negociation`
- **Duration**: 5 days (8 modules)
- **Price**: 245,000 FCFA (pack)

### 6. Service Client & Expérience
- **URL**: https://digischool.africa/b2c-learn.html?course=service-client-experience
- **Slug**: `service-client-experience`
- **Duration**: 3 days (8 modules)
- **Price**: 180,000 FCFA (pack)

### 7. RH & Performance
- **URL**: https://digischool.africa/b2c-learn.html?course=rh-performance
- **Slug**: `rh-performance`
- **Duration**: 4 days (8 modules)
- **Price**: 245,000 FCFA (pack)

### 8. Data & Reporting pour décideurs
- **URL**: https://digischool.africa/b2c-learn.html?course=data-reporting-decideurs
- **Slug**: `data-reporting-decideurs`
- **Duration**: 3 days (8 modules)
- **Price**: 180,000 FCFA (pack)

### 9. Productivité & Outils (Microsoft 365)
- **URL**: https://digischool.africa/b2c-learn.html?course=productivite-m365
- **Slug**: `productivite-m365`
- **Duration**: 2 days (8 modules)
- **Price**: 180,000 FCFA (pack)

---

## 🎨 NEW FEATURES (V2.0.0)

### Design & UX
- ✅ Warm, bright design system (white background, green/blue/amber accents)
- ✅ 9 course-specific illustrations (SVG)
- ✅ DigiSchool logo & branding
- ✅ Consistent header with social links
- ✅ Enhanced typography & spacing

### Conversion Features
- ✅ Social proof activity feed (live purchases/completions)
- ✅ Trust badges (100% online, certificat, support)
- ✅ Referral/parrainage system (earn vouchers)
- ✅ Auto-evaluation recommender tool
- ✅ Free chatbot (knowledge-based FAQ)
- ✅ WhatsApp assistant (for operators)

### Strict Rules
- ✅ Payment numbers ONLY in b2c-checkout.html (after cart)
- ✅ B2B never shows prices or payment numbers
- ✅ B2C 100% online (no downloads except certificates)
- ✅ Module gating preserved (70% quiz to unlock)

---

## 🔐 ADMIN ACCESS

### Admin Dashboard
- **URL**: https://digischool.africa/admin.html
- **Password**: DigiSchool2026!
- **Features**: Sales analytics, learner management, referral approvals, voucher issuance, CSV/JSON exports

### Admin Functions
1. View sales & revenue by course
2. Manage learners & progress
3. Approve/reject referrals
4. Issue vouchers
5. Export analytics data

---

## 📊 ANALYTICS EVENTS (New in V2)

### Conversion Events
- `ui_theme_refresh_loaded` - New design loaded
- `proof_social_view` - Social proof viewed
- `referral_code_copied` - User copied referral code
- `referral_code_used` - Referral code applied at checkout
- `voucher_applied` - Voucher discount applied
- `assessment_completed` - Auto-evaluation completed
- `chatbot_opened` - Chatbot panel opened
- `chatbot_escalate_whatsapp` - User escalated to WhatsApp
- `checkout_payment_section_view` - Payment methods viewed

---

## 🚀 LAUNCH INFORMATION

### V2.0.0 Launch
- **Date**: January 17, 2026
- **Status**: ✅ PRODUCTION READY
- **New Files**: 19 files created
- **Updated Files**: Multiple pages enhanced

### Key Improvements
1. **Warmer Design**: Bright, human-centered UI
2. **Social Proof**: Live activity feed + trust badges
3. **Referral System**: Client-side with admin validation
4. **Chatbot**: Free FAQ assistant
5. **Course Illustrations**: 9 unique SVG images
6. **Logo & Branding**: Professional DigiSchool identity

---

## 📞 SUPPORT & CONTACT

### For Learners (B2C)
- **WhatsApp**: +225 05 05 11 11 02
- **Email**: support@digischool.africa
- **Chatbot**: Available on all pages (click 💬 icon)

### For Businesses (B2B)
- **WhatsApp**: +225 05 05 11 11 02
- **Email**: contact@digischool.africa
- **Proforma Request**: Via companies.html form

### Technical Support
- **Admin Issues**: support@digischool.africa
- **Payment Confirmation**: Use WhatsApp or Email with transaction ID

---

## 🔗 PARTNER LINK

### Digilab (Parent Company)
- **Website**: https://www.mydigilab.io
- **Relationship**: DigiSchool Africa is powered by Digilab
- **Leadership**: Transformation digitale en Afrique de l'Ouest

---

## ✅ DEPLOYMENT CHECKLIST

### Pre-Deployment
- [ ] All new CSS/JS files uploaded
- [ ] Logo & illustrations in /assets/ directory
- [ ] Social media accounts created/verified
- [ ] Payment numbers tested
- [ ] Admin password secured
- [ ] Chatbot knowledge base updated

### Post-Deployment
- [ ] Test all social links
- [ ] Verify payment display rules (ONLY in checkout)
- [ ] Test referral code generation
- [ ] Test voucher redemption
- [ ] Verify chatbot responses
- [ ] Check mobile responsiveness
- [ ] Test admin dashboard
- [ ] Verify analytics events

---

## 📈 SUCCESS METRICS (V2.0.0)

### Conversion Improvements Expected
- **Social Proof**: +15% engagement
- **Referral System**: +10% organic growth
- **Chatbot**: -30% support tickets
- **Assessment Tool**: +20% course matching
- **Warm Design**: +25% time on site

---

**Last Updated**: January 17, 2026  
**Version**: 2.0.0  
**Prepared By**: GENSPARK.AI ONE-SHOT LOCKED SHIP  
**Status**: ✅ READY TO DEPLOY

---

🎓 **DigiSchool Africa** — *Transforming Professional Education in Africa*

Powered by **Digilab** 🚀
