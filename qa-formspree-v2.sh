...
# Test 5: Verify hidden fields (source, timestamp)
echo ""
echo "✓ TEST 5: Hidden tracking fields"
SOURCE_FOUND=0
TIMESTAMP_FOUND=0
[ $(grep -c 'name="source' reservation.html b2c-idea-box.html b2b-custom-request.html) -ge 3 ] && SOURCE_FOUND=1
[ $(grep -c 'name="timestamp"' reservation.html b2c-idea-box.html b2b-custom-request.html) -ge 3 ] && TIMESTAMP_FOUND=1
if [ "$SOURCE_FOUND" -eq 1 ] && [ "$TIMESTAMP_FOUND" -eq 1 ]; then
  echo "  ✅ PASS - source and timestamp fields present"
  ((PASS++))
else
  echo "  ❌ FAIL - Missing tracking fields"
  ((FAIL++))
fi
...
WA_FOUND=0
[ $(grep -c "wa.me" reservation-success.html idea-box-success.html custom-request-success.html) -ge 3 ] && WA_FOUND=1
if [ "$WA_FOUND" -eq 1 ]; then
