#!/usr/bin/env python3
"""V2.2.x-K - Inject Partner Logos Section"""
...
for page in PAGES:
    try:
        ...
        # Skip if already has partner section
        if 'ds-partners-section' in content:
            print(f"✓ {page} - Partner logos already present")
            continue
        
        # Inject before </body>
        if '</body>' in content:
            content = content.replace('</body>', f'{PARTNER_SECTION}\n</body>')
            with open(page, 'w', encoding='utf-8') as f:
                f.write(content)
            print(f"✓ {page} - Partner logos injected")
        else:
            print(f"⚠ {page} - No </body> tag found")
    except Exception as e:
        print(f"✗ {page} - Error: {e}")

print("\n✅ Partner Logos Injection Complete")
